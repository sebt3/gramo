...
import { RedisRedis } from './type.Redis.js';
import rfc6902  from 'rfc6902';

const customApi = kc.makeApiClient(k8s.CustomObjectsApi);
export const mutations = {
    createRedisRedis: async (_parent, args: object) => {
        const spec = {};
        ...
        try {
            const res = await customApi.createNamespacedCustomObject('redis.redis.opstreelabs.in','v1beta1',args['namespace'],'redis', payload)
    ...
    patchRedisRedis: async (_parent, args: object) => {
        const spec = {};
        if (args['spec'] == undefined || args['name']==undefined) return null;
        if (args['spec']['TLS'] !== undefined && typeof args['spec']['TLS'] == 'object') spec['TLS'] = args['spec']['TLS'];
        if (args['spec']['affinity'] !== undefined && typeof args['spec']['affinity'] == 'object') spec['affinity'] = args['spec']['affinity'];
        ...
        try {
            const resGet = await customApi.getNamespacedCustomObject('redis.redis.opstreelabs.in','v1beta1',args['namespace'],'redis', args['name'])
            const extGet = resGet.body as RedisRedis
            const payload = rfc6902.createPatch(extGet,request).filter(rule => !rule.path.startsWith('/status') && !['/metadata/creationTimestamp', '/metadata/finalizers', '/metadata/generation', '/metadata/managedFields', '/metadata/resourceVersion','/metadata/uid'].includes(rule.path) )
            const options = { "headers": { "Content-type": k8s.PatchUtils.PATCH_FORMAT_JSON_PATCH}};
            const res = await customApi.patchNamespacedCustomObject('redis.redis.opstreelabs.in','v1beta1',args['namespace'],'redis', args['name'], payload, undefined, undefined, undefined, options)
            const ext = res.body as RedisRedis
            return {
                metadata: getMetadata(ext.metadata),
                "TLS": ext.spec['TLS'],
    ...
    deleteRedisRedis: async (_parent, args: object) => {
    try {
        const res = await customApi.deleteNamespacedCustomObject('redis.redis.opstreelabs.in','v1beta1',args['namespace'],'redis', args['name'])
        const ext = res.body as RedisRedis
        return {
            metadata: getMetadata(ext.metadata),
            "TLS": ext.spec['TLS'],
