...
import { K8upCheck } from './type.Check.js';
import rfc6902  from 'rfc6902';

const customApi = kc.makeApiClient(k8s.CustomObjectsApi);
export const mutations = {
    createK8upCheck: async (_parent, args: object) => {
        const spec = {};
        ...
        try {
            const res = await customApi.createNamespacedCustomObject('k8up.io','v1',args['namespace'],'checks', payload)
    ...
    patchK8upCheck: async (_parent, args: object) => {
        const spec = {};
        if (args['spec'] == undefined || args['name']==undefined) return null;
        if (args['spec']['activeDeadlineSeconds'] !== undefined && typeof args['spec']['activeDeadlineSeconds'] == 'number') spec['activeDeadlineSeconds'] = args['spec']['activeDeadlineSeconds'];
        if (args['spec']['backend'] !== undefined && typeof args['spec']['backend'] == 'object') spec['backend'] = args['spec']['backend'];
        ...
        try {
            const resGet = await customApi.getNamespacedCustomObject('k8up.io','v1',args['namespace'],'checks', args['name'])
            const extGet = resGet.body as K8upCheck
            const payload = rfc6902.createPatch(extGet,request).filter(rule => !rule.path.startsWith('/status') && !['/metadata/creationTimestamp', '/metadata/finalizers', '/metadata/generation', '/metadata/managedFields', '/metadata/resourceVersion','/metadata/uid'].includes(rule.path) )
            const options = { "headers": { "Content-type": k8s.PatchUtils.PATCH_FORMAT_JSON_PATCH}};
            const res = await customApi.patchNamespacedCustomObject('k8up.io','v1',args['namespace'],'checks', args['name'], payload, undefined, undefined, undefined, options)
            const ext = res.body as K8upCheck
            return {
                metadata: getMetadata(ext.metadata),
                "activeDeadlineSeconds": ext.spec['activeDeadlineSeconds'],
    ...
    deleteK8upCheck: async (_parent, args: object) => {
    try {
        const res = await customApi.deleteNamespacedCustomObject('k8up.io','v1',args['namespace'],'checks', args['name'])
        const ext = res.body as K8upCheck
        return {
            metadata: getMetadata(ext.metadata),
            "activeDeadlineSeconds": ext.spec['activeDeadlineSeconds'],
