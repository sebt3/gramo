...
import { ref, useJob } from '../../libs/k8s/Job.js'
const { actionNew, toEdit, actionDelete } = useJob();
const to=ref({})
...
<template>
  <q-card bordered class="q-ma-sm">
    <q-card-section class="bg-primary text-grey-4">
      <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Job
    ...
    <q-card-section  v-if="useActions">
      <MetadataView
        ...
        :to="{ name: 'k8sJobTable', params: { namespace: model.metadata.namespace } }"
        />
    </q-card-section>
    <q-card-section  v-else>
      <MetadataView
        :metadata="model.metadata"
        :to="{ name: 'k8sJobTable', params: { namespace: model.metadata.namespace } }"
        :nameTo="{ name: 'k8sJobView', params: { namespace: model.metadata.namespace, name: model.metadata.name } }"
        />
    </q-card-section>
  </q-card>
</template>
