...
import { ref, usePod } from '../../libs/k8s/Pod.js'
const { actionNew, toEdit, actionDelete } = usePod();
const to=ref({})
...
<template>
  <q-card bordered class="q-ma-sm">
    <q-card-section class="bg-primary text-grey-4">
      <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Pod
    ...
    <q-card-section  v-if="useActions">
      <MetadataView
        ...
        :to="{ name: 'k8sPodTable', params: { namespace: model.metadata.namespace } }"
        />
    </q-card-section>
    <q-card-section  v-else>
      <MetadataView
        :metadata="model.metadata"
        :to="{ name: 'k8sPodTable', params: { namespace: model.metadata.namespace } }"
        :nameTo="{ name: 'k8sPodView', params: { namespace: model.metadata.namespace, name: model.metadata.name } }"
        />
    </q-card-section>
  </q-card>
</template>
