...
export function useStringSecret() {
  ...
  return {
    ...
    onNotStringSecretFound: (res) => {
      if ( !res.loading && res.data.secretgeneratorStringSecret == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> StringSecret <strong>${row.metadata.name}</strong>`,
