...
export function useSSHKeyPair() {
  ...
  return {
    ...
    onNotSSHKeyPairFound: (res) => {
      if ( !res.loading && res.data.secretgeneratorSSHKeyPair == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> SSHKeyPair <strong>${row.metadata.name}</strong>`,
