...
import { ref, useSSHKeyPair, SSHKeyPairColumns, SSHKeyPairAllColumns } from '../../libs/secretgenerator/SSHKeyPair.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useSSHKeyPair();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="secretgenerator SSHKeyPairs" :usecreate="useAction" :userefresh="useAction" itemtype='secretgenerator SSHKeyPair' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?SSHKeyPairAllColumns:SSHKeyPairColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
