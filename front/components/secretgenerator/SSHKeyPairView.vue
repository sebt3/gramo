...
const { result, loading, onResult, onError } = useQuery(secretgeneratorSSHKeyPairQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotSSHKeyPairFound(res);viewerUpdate(res, res.data.secretgeneratorSSHKeyPair.metadata.obj)});
