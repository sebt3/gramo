...
const { result, loading, onResult, onError } = useQuery(secretgeneratorBasicAuthQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotBasicAuthFound(res);viewerUpdate(res, res.data.secretgeneratorBasicAuth.metadata.obj)});
