...
export function useBasicAuth() {
  ...
  return {
    ...
    onNotBasicAuthFound: (res) => {
      if ( !res.loading && res.data.secretgeneratorBasicAuth == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> BasicAuth <strong>${row.metadata.name}</strong>`,
