...
import { useQuery, useMutation, useStringSecret, getProperties } from './StringSecret.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotStringSecretFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useStringSecret();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(secretgeneratorStringSecretQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotStringSecretFound(res);viewerUpdate(res, res.loading?{}:res.data.secretgeneratorStringSecret.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.secretgeneratorStringSecret!=undefined && result.secretgeneratorStringSecret!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">StringSecret
