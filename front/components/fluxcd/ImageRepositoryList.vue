...
import { ref, useImageRepository, ImageRepositoryColumns, ImageRepositoryAllColumns } from '../../libs/fluxcd/ImageRepository.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useImageRepository();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd ImageRepositorys" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd ImageRepository' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ImageRepositoryAllColumns:ImageRepositoryColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
