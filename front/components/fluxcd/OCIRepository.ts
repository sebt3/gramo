...
export function useOCIRepository() {
  ...
  return {
    ...
    onNotOCIRepositoryFound: (res) => {
      if ( !res.loading && res.data.fluxcdOCIRepository == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> OCIRepository <strong>${row.metadata.name}</strong>`,
