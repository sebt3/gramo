...
import { ref, useGitRepository, GitRepositoryColumns, GitRepositoryAllColumns } from '../../libs/fluxcd/GitRepository.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useGitRepository();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd GitRepositorys" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd GitRepository' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?GitRepositoryAllColumns:GitRepositoryColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
