...
import { ref, useHelmRelease, HelmReleaseColumns, HelmReleaseAllColumns } from '../../libs/fluxcd/HelmRelease.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useHelmRelease();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd HelmReleases" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd HelmRelease' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?HelmReleaseAllColumns:HelmReleaseColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
