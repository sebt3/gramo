...
] as Array<QTableColumn>).concat(kustomizationColumns);
...
export function useKustomization() {
  ...
  return {
    ...
    onNotKustomizationFound: (res) => {
      if ( !res.loading && res.data.fluxcdKustomization == null) {
        const matched = router.currentRoute.value.matched
        notifyError('Kustomization not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'fluxcdKustomizationView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'fluxcdKustomizationView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'fluxcdKustomizationEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'fluxcdKustomizationCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> Kustomization <strong>${row.metadata.name}</strong>`,
