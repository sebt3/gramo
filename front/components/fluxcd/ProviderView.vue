...
import { ref, useQuery, useMutation, useProvider, packageColumns, getProperties } from './Provider.js'
let filter = ref('');
const { onErrorHandler, notifySuccess, notifyError, onNotProviderFound, navigation, pagination, setNamespacedItemFromRoute, toEdit, actionDelete } = useProvider();setNamespacedItemFromRoute();
...
<template>
  ...
  <q-card v-if="!loading && result!=undefined && result.fluxcdProvider!=undefined && result.fluxcdProvider!=null" class="q-ma-sm">
    <q-card-section>
      <TableHeader title="Packages" v-model:model-filter="filter" itemtype='Package' :usecreate="false" :userefresh="false" />
      <q-table :rows="result.fluxcdProvider.packages" :columns="packageColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom></q-table>
    </q-card-section>
  </q-card>
</template>
