...
import { ref, useReceiver, ReceiverColumns, ReceiverAllColumns } from '../../libs/fluxcd/Receiver.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useReceiver();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd Receivers" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd Receiver' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ReceiverAllColumns:ReceiverColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
