...
import { ref, useImageUpdateAutomation, ImageUpdateAutomationColumns, ImageUpdateAutomationAllColumns } from '../../libs/fluxcd/ImageUpdateAutomation.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useImageUpdateAutomation();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd ImageUpdateAutomations" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd ImageUpdateAutomation' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ImageUpdateAutomationAllColumns:ImageUpdateAutomationColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
