...
const { result, loading, onResult, onError } = useQuery(fluxcdImageRepositoryQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotImageRepositoryFound(res);viewerUpdate(res, res.data.fluxcdImageRepository.metadata.obj)});
