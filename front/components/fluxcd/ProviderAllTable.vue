...
import fluxcdAllProvidersQuery from '@/queries/fluxcd/ProviderAllTable.graphql'
import TableHeader from '../core/TableHeader.vue';
import { ref, useQuery, useProvider, providerAllColumns } from './Provider.js'
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = useProvider();
const { result, refetch, onError } = useQuery(fluxcdAllProvidersQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="Fluxcd Providers" v-model:model-filter="filter" itemtype='fluxcd Provider' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.fluxcdProviders).flat()" :columns="providerAllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
