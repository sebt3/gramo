...
const { viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotImageUpdateAutomationFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useImageUpdateAutomation();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(fluxcdImageUpdateAutomationQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotImageUpdateAutomationFound(res);viewerUpdate(res, res.data.fluxcdImageUpdateAutomation.metadata.obj)});
