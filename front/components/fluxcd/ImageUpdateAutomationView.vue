...
import { ref, useQuery, useMutation, useImageUpdateAutomation, packageColumns, getProperties } from './ImageUpdateAutomation.js'
let filter = ref('');
const { onErrorHandler, notifySuccess, notifyError, onNotImageUpdateAutomationFound, navigation, pagination, setNamespacedItemFromRoute, toEdit, actionDelete } = useImageUpdateAutomation();setNamespacedItemFromRoute();
...
<template>
  ...
  <q-card v-if="!loading && result!=undefined && result.fluxcdImageUpdateAutomation!=undefined && result.fluxcdImageUpdateAutomation!=null" class="q-ma-sm">
    <q-card-section>
      <TableHeader title="Packages" v-model:model-filter="filter" itemtype='Package' :usecreate="false" :userefresh="false" />
      <q-table :rows="result.fluxcdImageUpdateAutomation.packages" :columns="packageColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom></q-table>
    </q-card-section>
  </q-card>
</template>
