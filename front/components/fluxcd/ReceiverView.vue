...
import { useQuery, useMutation, useReceiver, getProperties } from './Receiver.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotReceiverFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useReceiver();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(fluxcdReceiverQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotReceiverFound(res);viewerUpdate(res, res.loading?{}:res.data.fluxcdReceiver.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.fluxcdReceiver!=undefined && result.fluxcdReceiver!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Receiver
