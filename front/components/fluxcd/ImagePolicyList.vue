...
import { ref, useImagePolicy, ImagePolicyColumns, ImagePolicyAllColumns } from '../../libs/fluxcd/ImagePolicy.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useImagePolicy();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd ImagePolicys" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd ImagePolicy' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ImagePolicyAllColumns:ImagePolicyColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
