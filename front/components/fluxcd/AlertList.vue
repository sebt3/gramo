...
import { ref, useAlert, AlertColumns, AlertAllColumns } from '../../libs/fluxcd/Alert.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useAlert();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd Alerts" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd Alert' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?AlertAllColumns:AlertColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
