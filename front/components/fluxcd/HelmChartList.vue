...
import { ref, useHelmChart, HelmChartColumns, HelmChartAllColumns } from '../../libs/fluxcd/HelmChart.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useHelmChart();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd HelmCharts" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd HelmChart' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?HelmChartAllColumns:HelmChartColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
