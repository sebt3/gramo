...
import { ref, useQuery, useMutation, useImagePolicy, packageColumns, getProperties } from './ImagePolicy.js'
let filter = ref('');
const { onErrorHandler, notifySuccess, notifyError, onNotImagePolicyFound, navigation, pagination, setNamespacedItemFromRoute, toEdit, actionDelete } = useImagePolicy();setNamespacedItemFromRoute();
...
<template>
  ...
  <q-card v-if="!loading && result!=undefined && result.fluxcdImagePolicy!=undefined && result.fluxcdImagePolicy!=null" class="q-ma-sm">
    <q-card-section>
      <TableHeader title="Packages" v-model:model-filter="filter" itemtype='Package' :usecreate="false" :userefresh="false" />
      <q-table :rows="result.fluxcdImagePolicy.packages" :columns="packageColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom></q-table>
    </q-card-section>
  </q-card>
</template>
