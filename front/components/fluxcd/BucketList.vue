...
import { ref, useBucket, BucketColumns, BucketAllColumns } from '../../libs/fluxcd/Bucket.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useBucket();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd Buckets" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd Bucket' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?BucketAllColumns:BucketColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
