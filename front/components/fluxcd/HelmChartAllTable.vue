...
import fluxcdAllHelmChartsQuery from '@/queries/fluxcd/HelmChartAllTable.graphql'
import TableHeader from '../core/TableHeader.vue';
import { ref, useQuery, useHelmChart, helmChartAllColumns } from './HelmChart.js'
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = useHelmChart();
const { result, refetch, onError } = useQuery(fluxcdAllHelmChartsQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="Fluxcd HelmCharts" v-model:model-filter="filter" itemtype='fluxcd HelmChart' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.fluxcdHelmCharts).flat()" :columns="helmChartAllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
