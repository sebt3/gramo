...
import { ref, useProvider, ProviderColumns, ProviderAllColumns } from '../../libs/fluxcd/Provider.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useProvider();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd Providers" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd Provider' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ProviderAllColumns:ProviderColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
