...
import fluxcdAllReceiversQuery from '@/queries/fluxcd/ReceiverAllTable.graphql'
import TableHeader from '../core/TableHeader.vue';
import { ref, useQuery, useReceiver, receiverAllColumns } from './Receiver.js'
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = useReceiver();
const { result, refetch, onError } = useQuery(fluxcdAllReceiversQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="Fluxcd Receivers" v-model:model-filter="filter" itemtype='fluxcd Receiver' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.fluxcdReceivers).flat()" :columns="receiverAllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
