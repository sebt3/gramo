...
export function useHelmRepository() {
  ...
  return {
    ...
    onNotHelmRepositoryFound: (res) => {
      if ( !res.loading && res.data.fluxcdHelmRepository == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> HelmRepository <strong>${row.metadata.name}</strong>`,
