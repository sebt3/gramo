...
import { ref, useHelmRepository, HelmRepositoryColumns, HelmRepositoryAllColumns } from '../../libs/fluxcd/HelmRepository.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useHelmRepository();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="fluxcd HelmRepositorys" :usecreate="useAction" :userefresh="useAction" itemtype='fluxcd HelmRepository' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?HelmRepositoryAllColumns:HelmRepositoryColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
