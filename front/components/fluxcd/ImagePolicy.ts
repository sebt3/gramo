...
export function useImagePolicy() {
  ...
  return {
    ...
    onNotImagePolicyFound: (res) => {
      if ( !res.loading && res.data.fluxcdImagePolicy == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> ImagePolicy <strong>${row.metadata.name}</strong>`,
