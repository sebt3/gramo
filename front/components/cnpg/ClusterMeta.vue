...
import { ref, useCluster } from '../../libs/cnpg/Cluster.js'
const { actionNew, toEdit, actionDelete } = useCluster();
const to=ref({})
...
<template>
  <q-card bordered class="q-ma-sm">
    <q-card-section class="bg-primary text-grey-4">
      <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Cluster
    ...
    <q-card-section  v-if="useActions">
      <MetadataView
        ...
        :to="{ name: 'cnpgClusterTable', params: { namespace: model.metadata.namespace } }"
        />
    </q-card-section>
    <q-card-section  v-else>
      <MetadataView
        :metadata="model.metadata"
        :to="{ name: 'cnpgClusterTable', params: { namespace: model.metadata.namespace } }"
        :nameTo="{ name: 'cnpgClusterView', params: { namespace: model.metadata.namespace, name: model.metadata.name } }"
        />
    </q-card-section>
  </q-card>
</template>
