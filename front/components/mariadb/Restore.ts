...
export function useRestore() {
  ...
  return {
    ...
    onNotRestoreFound: (res) => {
      if ( !res.loading && res.data.mariadbRestore == null) {
    ...
    toView: (namespace, name) => router.push({ name: 'mariadbRestoreView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'mariadbRestoreView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'mariadbRestoreEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'mariadbRestoreCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> Restore <strong>${row.metadata.name}</strong>`,
