...
import mariadbAllDatabasesQuery from '@/queries/mariadb/DatabaseAllTable.graphql'
import TableHeader from '../core/TableHeader.vue';
import { ref, useQuery, useDatabase, databaseAllColumns } from './Database.js'
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = useDatabase();
const { result, refetch, onError } = useQuery(mariadbAllDatabasesQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="Mariadb Databases" v-model:model-filter="filter" itemtype='mariadb Database' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.mariadbDatabases).flat()" :columns="databaseAllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
