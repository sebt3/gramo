...
import { ref, useConnection, ConnectionColumns, ConnectionAllColumns } from '../../libs/mariadb/Connection.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useConnection();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="mariadb Connections" :usecreate="useAction" :userefresh="useAction" itemtype='mariadb Connection' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ConnectionAllColumns:ConnectionColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
