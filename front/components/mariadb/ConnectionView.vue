...
import { ref, useQuery, useMutation, useConnection, getProperties } from './Connection.js'
const { onErrorHandler, notifySuccess, notifyError, onNotConnectionFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useConnection();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(mariadbConnectionQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotConnectionFound);
  ...
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.mariadbConnection!=undefined && result.mariadbConnection!=null" class="q-ma-sm">
        <q-card-section>
          <div class="text-h6 text-grey-8 q-mt-none q-mb-none q-pt-none q-pb-none">Connection
