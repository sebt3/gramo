...
import { ref, useSqlJob, SqlJobColumns, SqlJobAllColumns } from '../../libs/mariadb/SqlJob.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useSqlJob();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="mariadb SqlJobs" :usecreate="useAction" :userefresh="useAction" itemtype='mariadb SqlJob' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?SqlJobAllColumns:SqlJobColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
