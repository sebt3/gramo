...
const { result, loading, onResult, onError } = useQuery(mariadbMariaDBQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotMariaDBFound(res);viewerUpdate(res, res.data.mariadbMariaDB.metadata.obj)});
