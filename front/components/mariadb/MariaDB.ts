...
export const mariaDBColumns:Array<QTableColumn> = [
...
export const mariaDBAllColumns = ([
...
export function useMariaDB() {
  ...
  return {
    ...
    toView: (namespace, name) => router.push({ name: 'mariadbMariaDBView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'mariadbMariaDBView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'mariadbMariaDBEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'mariadbMariaDBCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> MariaDB <strong>${row.metadata.name}</strong>`,
