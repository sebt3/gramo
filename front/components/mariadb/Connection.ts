...
export function useConnection() {
  ...
  return {
    ...
    onNotConnectionFound: (res) => {
      if ( !res.loading && res.data.mariadbConnection == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> Connection <strong>${row.metadata.name}</strong>`,
