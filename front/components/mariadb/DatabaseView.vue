...
const { onErrorHandler, notifySuccess, notifyError, onNotDatabaseFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useDatabase();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(mariadbDatabaseQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotDatabaseFound);
