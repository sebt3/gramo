...
import { useQuery, useMutation, useSqlJob, getProperties } from './SqlJob.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotSqlJobFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useSqlJob();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(mariadbSqlJobQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotSqlJobFound(res);viewerUpdate(res, res.loading?{}:res.data.mariadbSqlJob.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.mariadbSqlJob!=undefined && result.mariadbSqlJob!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">SqlJob
