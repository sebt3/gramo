...
const { result, loading, onResult, onError } = useQuery(mariadbBackupQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotBackupFound(res);viewerUpdate(res, res.loading?{}:res.data.mariadbBackup.metadata.obj)});
