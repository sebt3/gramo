...
const { result, loading, onResult, onError } = useQuery(rabbitmqRabbitmqClusterQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotRabbitmqClusterFound(res);viewerUpdate(res, res.data.rabbitmqRabbitmqCluster.metadata.obj)});
