...
import { ref, useRabbitmqCluster, RabbitmqClusterColumns, RabbitmqClusterAllColumns } from '../../libs/rabbitmq/RabbitmqCluster.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useRabbitmqCluster();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="rabbitmq RabbitmqClusters" :usecreate="useAction" :userefresh="useAction" itemtype='rabbitmq RabbitmqCluster' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?RabbitmqClusterAllColumns:RabbitmqClusterColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
