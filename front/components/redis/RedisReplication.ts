...
export const redisReplicationColumns:Array<QTableColumn> = [
  {name: 'Name', label: 'Name', field: row => row.metadata.name, sortable: true, align: tableColumnAlign.left},
  {name: 'Action', label: '', field: 'Action', sortable: false, align: tableColumnAlign.right}
];
export const redisReplicationAllColumns = ([
  ...
  return {
    ...
    onNotRedisReplicationFound: (res) => {
      if ( !res.loading && res.data.redisRedisReplication == null) {
        ...
        notifyError('RedisReplication not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'redisRedisReplicationView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'redisRedisReplicationView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'redisRedisReplicationEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'redisRedisReplicationCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> RedisReplication <strong>${row.metadata.name}</strong>`,
