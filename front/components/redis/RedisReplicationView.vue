...
import { useQuery, useMutation, useRedisReplication, getProperties } from './RedisReplication.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotRedisReplicationFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useRedisReplication();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(redisRedisReplicationQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotRedisReplicationFound(res);viewerUpdate(res, res.loading?{}:res.data.redisRedisReplication.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.redisRedisReplication!=undefined && result.redisRedisReplication!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">RedisReplication
