...
import { ref, useRedisCluster, RedisClusterColumns, RedisClusterAllColumns } from '../../libs/redis/RedisCluster.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useRedisCluster();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="redis RedisClusters" :usecreate="useAction" :userefresh="useAction" itemtype='redis RedisCluster' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?RedisClusterAllColumns:RedisClusterColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
