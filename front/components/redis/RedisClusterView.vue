...
const { result, loading, onResult, onError } = useQuery(redisRedisClusterQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotRedisClusterFound(res);viewerUpdate(res, res.data.redisRedisCluster.metadata.obj)});
