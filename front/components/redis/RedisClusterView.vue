...
import { useQuery, useMutation, useRedisCluster, getProperties } from './RedisCluster.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotRedisClusterFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useRedisCluster();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(redisRedisClusterQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotRedisClusterFound(res);viewerUpdate(res, res.loading?{}:res.data.redisRedisCluster.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.redisRedisCluster!=undefined && result.redisRedisCluster!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">RedisCluster
