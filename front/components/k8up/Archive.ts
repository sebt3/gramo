...
export const archiveColumns:Array<QTableColumn> = [
  {name: 'Name', label: 'Name', field: row => row.metadata.name, sortable: true, align: tableColumnAlign.left},
  {name: 'Action', label: '', field: 'Action', sortable: false, align: tableColumnAlign.right}
];
export const archiveAllColumns = ([
...
export function useArchive() {
  ...
  return {
    ...
    toView: (namespace, name) => router.push({ name: 'k8upArchiveView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8upArchiveView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8upArchiveEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8upArchiveCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> Archive <strong>${row.metadata.name}</strong>`,
