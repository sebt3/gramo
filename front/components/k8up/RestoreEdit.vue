...
import restoreEdit from '@/queries/k8up/RestoreEdit.graphql'
...
function onSubmit() {
  ...
  patchRestore({"namespace": result.value.k8upRestore.metadata.namespace, "name": result.value.k8upRestore.metadata.name, "spec": sanitizeData(data.value)});
}
...
<template>
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-sm-8 col-md-6">
      <q-card bordered v-if="!loading && result.k8upRestore!=null" class="q-ma-sm">
        <q-card-section>
  ...
  <q-form @submit="onSubmit" class="q-gutter-md q-pt-sm q-ml-sm">
    <q-card bordered v-if="!loading && editor.ready && result.k8upRestore!=null" class="q-ma-sm">
