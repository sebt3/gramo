...
import pruneNew from '@/queries/k8up/PruneNew.graphql'
...
const name = ref('');
const data = ref({});
const { router, navigation, setNamespacedItemFromRoute, notifySuccess, notifyError, notifyWorking } = usePrune();setNamespacedItemFromRoute();
const { result, loading } = useQuery(customResourceDefinition, {"name": 'prunes.k8up.io'});
const { mutate, onDone, onError } = useMutation(pruneNew);
onDone(() => {
