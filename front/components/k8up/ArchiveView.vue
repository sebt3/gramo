...
import { ref, useQuery, useMutation, useArchive, getProperties } from './Archive.js'
const { onErrorHandler, notifySuccess, notifyError, onNotArchiveFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useArchive();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(k8upArchiveQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotArchiveFound);
  ...
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.k8upArchive!=undefined && result.k8upArchive!=null" class="q-ma-sm">
        <q-card-section>
          <div class="text-h6 text-grey-8 q-mt-none q-mb-none q-pt-none q-pb-none">Archive
