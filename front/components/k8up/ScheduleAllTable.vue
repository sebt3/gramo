...
import k8upAllSchedulesQuery from '@/queries/k8up/ScheduleAllTable.graphql'
import TableHeader from '../core/TableHeader.vue';
import { ref, useQuery, useSchedule, scheduleAllColumns } from './Schedule.js'
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = useSchedule();
const { result, refetch, onError } = useQuery(k8upAllSchedulesQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="K8up Schedules" v-model:model-filter="filter" itemtype='k8up Schedule' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.k8upSchedules).flat()" :columns="scheduleAllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
