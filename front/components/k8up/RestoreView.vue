...
const { result, loading, onResult, onError } = useQuery(k8upRestoreQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotRestoreFound(res);viewerUpdate(res, res.loading?{}:res.data.k8upRestore.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.k8upRestore!=undefined && result.k8upRestore!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Restore
