...
export const scheduleColumns:Array<QTableColumn> = [
  {name: 'Name', label: 'Name', field: row => row.metadata.name, sortable: true, align: tableColumnAlign.left},
  {name: 'Action', label: '', field: 'Action', sortable: false, align: tableColumnAlign.right}
];
export const scheduleAllColumns = ([
  ...
  return {
    ...
    onNotScheduleFound: (res) => {
      if ( !res.loading && res.data.k8upSchedule == null) {
        ...
        notifyError('Schedule not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'k8upScheduleView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8upScheduleView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8upScheduleEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8upScheduleCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> Schedule <strong>${row.metadata.name}</strong>`,
