...
import { useQuery, useMutation, useMongoDBCommunity, getProperties } from './MongoDBCommunity.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotMongoDBCommunityFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useMongoDBCommunity();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(mongodbcommunityMongoDBCommunityQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotMongoDBCommunityFound(res);viewerUpdate(res, res.loading?{}:res.data.mongodbcommunityMongoDBCommunity.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.mongodbcommunityMongoDBCommunity!=undefined && result.mongodbcommunityMongoDBCommunity!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">MongoDBCommunity
