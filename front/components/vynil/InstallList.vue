...
import { ref, useInstall, InstallColumns, InstallAllColumns } from '../../libs/vynil/Install.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useInstall();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="vynil Installs" :usecreate="useAction" :userefresh="useAction" itemtype='vynil Install' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?InstallAllColumns:InstallColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
