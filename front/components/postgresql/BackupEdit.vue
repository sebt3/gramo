...
import backupEdit from '@/queries/postgresql/BackupEdit.graphql'
...
const { result, loading, onResult, onError } = useQuery(postgresqlBackupQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem}, { pollInterval: 500 });
const { mutate: patchBackup, onDone: onPatchBackup, onError: onPatchError } = useMutation(backupEdit);
function onSubmit() {
  notifyWorking('Update in progress');
  patchBackup({"namespace": result.value.postgresqlBackup.metadata.namespace, "name": result.value.postgresqlBackup.metadata.name, "spec": sanitizeData(data.value)});
}
onError(onErrorHandler);onResult(res => {onNotBackupFound(res);editor.value.updateFromQuery(res, res.loading?{}:{spec: res.data.postgresqlBackup.metadata.obj.spec})});onPatchBackup(patchDone);onPatchError(patchError);
</script>
<template>
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-sm-8 col-md-6">
      <q-card bordered v-if="!loading && result.postgresqlBackup!=null" class="q-ma-sm">
        <q-card-section>
  ...
  <q-form @submit="onSubmit" class="q-gutter-md q-pt-sm q-ml-sm">
    <q-card bordered v-if="!loading && editor.ready && result.postgresqlBackup!=null" class="q-ma-sm">
