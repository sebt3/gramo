...
const { onErrorHandler, notifySuccess, notifyError, onNotPoolerFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = usePooler();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(postgresqlPoolerQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotPoolerFound);
