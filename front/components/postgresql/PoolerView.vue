...
import { useQuery, useMutation, usePooler, getProperties } from './Pooler.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotPoolerFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = usePooler();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(postgresqlPoolerQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotPoolerFound(res);viewerUpdate(res, res.loading?{}:res.data.postgresqlPooler.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.postgresqlPooler!=undefined && result.postgresqlPooler!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Pooler
