...
const { result, loading, onResult, onError } = useQuery(postgresqlBackupQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotBackupFound(res);viewerUpdate(res, res.data.postgresqlBackup.metadata.obj)});
