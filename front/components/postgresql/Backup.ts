...
export function useBackup() {
  ...
  return {
    ...
    onNotBackupFound: (res) => {
      if ( !res.loading && res.data.postgresqlBackup == null) {
    ...
    toView: (namespace, name) => router.push({ name: 'postgresqlBackupView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'postgresqlBackupView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'postgresqlBackupEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'postgresqlBackupCreate', params: {namespace}}),
