...
const { result, loading, onResult, onError } = useQuery(projectcalicoGlobalNetworkPolicyQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotGlobalNetworkPolicyFound(res);viewerUpdate(res, res.data.projectcalicoGlobalNetworkPolicy.metadata.obj)});
