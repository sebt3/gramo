...
const { onErrorHandler, notifySuccess, notifyError, onNotIPPoolFound, navigation, setItemFromRoute, toEdit, actionDelete } = useIPPool();setItemFromRoute();
const { result, loading, onResult, onError } = useQuery(projectcalicoIPPoolQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotIPPoolFound);
