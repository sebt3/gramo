...
import { ref, useQuery, useMutation, useIPPool, getProperties } from './IPPool.js'
const { viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotIPPoolFound, navigation, setItemFromRoute, toEdit, actionDelete } = useIPPool();setItemFromRoute();
const { result, loading, onResult, onError } = useQuery(projectcalicoIPPoolQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotIPPoolFound(res);viewerUpdate(res, res.data.projectcalicoIPPool.metadata.obj)});
  ...
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.projectcalicoIPPool!=undefined && result.projectcalicoIPPool!=null" class="q-ma-sm">
        <q-card-section>
          <div class="text-h6 text-grey-8 q-mt-none q-mb-none q-pt-none q-pb-none">IPPool
