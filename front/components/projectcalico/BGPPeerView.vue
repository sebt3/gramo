...
const { result, loading, onResult, onError } = useQuery(projectcalicoBGPPeerQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotBGPPeerFound(res);viewerUpdate(res, res.data.projectcalicoBGPPeer.metadata.obj)});
