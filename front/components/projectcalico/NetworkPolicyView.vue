...
import { ref, useQuery, useMutation, useNetworkPolicy, getProperties } from './NetworkPolicy.js'
const { viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotNetworkPolicyFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useNetworkPolicy();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(projectcalicoNetworkPolicyQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotNetworkPolicyFound(res);viewerUpdate(res, res.data.projectcalicoNetworkPolicy.metadata.obj)});
  ...
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.projectcalicoNetworkPolicy!=undefined && result.projectcalicoNetworkPolicy!=null" class="q-ma-sm">
        <q-card-section>
          <div class="text-h6 text-grey-8 q-mt-none q-mb-none q-pt-none q-pb-none">NetworkPolicy
