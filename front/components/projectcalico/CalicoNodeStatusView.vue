...
const { onErrorHandler, notifySuccess, notifyError, onNotCalicoNodeStatusFound, navigation, setItemFromRoute, toEdit, actionDelete } = useCalicoNodeStatus();setItemFromRoute();
const { result, loading, onResult, onError } = useQuery(projectcalicoCalicoNodeStatusQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotCalicoNodeStatusFound);
