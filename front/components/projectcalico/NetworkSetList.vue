...
import { ref, useNetworkSet, NetworkSetColumns, NetworkSetAllColumns } from '../../libs/projectcalico/NetworkSet.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useNetworkSet();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="projectcalico NetworkSets" :usecreate="useAction" :userefresh="useAction" itemtype='projectcalico NetworkSet' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?NetworkSetAllColumns:NetworkSetColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
