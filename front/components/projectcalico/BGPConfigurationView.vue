...
const { onErrorHandler, notifySuccess, notifyError, onNotBGPConfigurationFound, navigation, setItemFromRoute, toEdit, actionDelete } = useBGPConfiguration();setItemFromRoute();
const { result, loading, onResult, onError } = useQuery(projectcalicoBGPConfigurationQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(onNotBGPConfigurationFound);
