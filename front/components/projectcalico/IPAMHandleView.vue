...
const { result, loading, onResult, onError } = useQuery(projectcalicoIPAMHandleQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotIPAMHandleFound(res);viewerUpdate(res, res.data.projectcalicoIPAMHandle.metadata.obj)});
