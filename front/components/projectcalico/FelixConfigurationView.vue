...
const { result, loading, onResult, onError } = useQuery(projectcalicoFelixConfigurationQuery, { "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotFelixConfigurationFound(res);viewerUpdate(res, res.data.projectcalicoFelixConfiguration.metadata.obj)});
