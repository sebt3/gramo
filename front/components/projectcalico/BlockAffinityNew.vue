...
import blockAffinityNew from '@/queries/projectcalico/BlockAffinityNew.graphql'
...
const name = ref('');
const data = ref({});
const { router, navigation, setItemFromRoute, notifySuccess, notifyError, notifyWorking } = useBlockAffinity();setItemFromRoute();
const { result, loading } = useQuery(customResourceDefinition, {"name": 'blockaffinities.crd.projectcalico.org'});
const { mutate, onDone, onError } = useMutation(blockAffinityNew);
onDone(() => {
