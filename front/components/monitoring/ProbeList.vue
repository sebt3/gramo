...
import { ref, useProbe, ProbeColumns, ProbeAllColumns } from '../../libs/monitoring/Probe.js'
const { actionNew, toEdit, actionDelete, pagination, navigation, isNamespaced } = useProbe();
let filter = ref('');
defineEmits(['refresh']);
...
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="monitoring Probes" :usecreate="useAction" :userefresh="useAction" itemtype='monitoring Probe' v-model:model-filter="filter" @refresh="$emit('refresh')" @create="actionNew(navigation.currentNamespace.value)" />
    <q-card-section class="q-pa-none">
    <q-table :rows="model" :columns="!isNamespaced()?ProbeAllColumns:ProbeColumns.filter(c=>c.name!='Action'||useAction)" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
