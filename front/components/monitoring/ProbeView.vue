...
import { ref, useQuery, useMutation, useProbe, getProperties } from './Probe.js'
const { viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotProbeFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useProbe();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(monitoringProbeQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotProbeFound(res);viewerUpdate(res, res.data.monitoringProbe.metadata.obj)});
  ...
  <div class="row q-mb-sm q-ml-sm">
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.monitoringProbe!=undefined && result.monitoringProbe!=null" class="q-ma-sm">
        <q-card-section>
          <div class="text-h6 text-grey-8 q-mt-none q-mb-none q-pt-none q-pb-none">Probe
