...
const { result, loading, onResult, onError } = useQuery(monitoringPrometheusRuleQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotPrometheusRuleFound(res);viewerUpdate(res, res.data.monitoringPrometheusRule.metadata.obj)});
