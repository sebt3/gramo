...
import { useQuery, useMutation, usePrometheus, getProperties } from './Prometheus.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotPrometheusFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = usePrometheus();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(monitoringPrometheusQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotPrometheusFound(res);viewerUpdate(res, res.loading?{}:res.data.monitoringPrometheus.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.monitoringPrometheus!=undefined && result.monitoringPrometheus!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">Prometheus
