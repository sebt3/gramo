...
import { useQuery, useMutation, useAlertmanagerConfig, getProperties } from './AlertmanagerConfig.js'
const { editor, viewer, viewerUpdate, onErrorHandler, notifySuccess, notifyError, onNotAlertmanagerConfigFound, navigation, setNamespacedItemFromRoute, toEdit, actionDelete } = useAlertmanagerConfig();setNamespacedItemFromRoute();
const { result, loading, onResult, onError } = useQuery(monitoringAlertmanagerConfigQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotAlertmanagerConfigFound(res);viewerUpdate(res, res.loading?{}:res.data.monitoringAlertmanagerConfig.metadata.obj)});
    ...
    <div class="col-md-6">
      <q-card bordered v-if="!loading && result!=undefined && result.monitoringAlertmanagerConfig!=undefined && result.monitoringAlertmanagerConfig!=null" class="q-ma-sm">
        <q-card-section class="bg-primary text-grey-4">
          <div class="text-h6 q-mt-none q-mb-none q-pt-none q-pb-none">AlertmanagerConfig
