...
const { result, loading, onResult, onError } = useQuery(monitoringAlertmanagerConfigQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotAlertmanagerConfigFound(res);viewerUpdate(res, res.data.monitoringAlertmanagerConfig.metadata.obj)});
