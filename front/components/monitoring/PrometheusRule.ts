...
export function usePrometheusRule() {
  ...
  return {
    ...
    onNotPrometheusRuleFound: (res) => {
      if ( !res.loading && res.data.monitoringPrometheusRule == null) {
    ...
    actionDelete: (deletor, row) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> PrometheusRule <strong>${row.metadata.name}</strong>`,
