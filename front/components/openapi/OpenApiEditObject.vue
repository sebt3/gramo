...
const {getProperties,getItems,getType, getFullData} = await import('../../libs/core/openapiSetup');
    ...
    <div class="q-gutter-md column q-ml-sm">
      <div v-for="[key, value] in new Map([...properties.entries()].filter(([key,value]) => (showdefault || !isDefault(key)) && (Object.keys(out).includes(key)||!readOnly)))" v-bind:key="key"
        ...
        <div v-else-if="value.type == 'number' && (Object.keys(out).includes(key)||!readOnly)">
          <OpenApiEditNumber v-model:data="out[key]" :name="key" :required="Array.isArray(required) && required.includes(key)" :defaultdata="value.default" :description="value.description" :read-only="readOnly" />
        </div>
        ...
        <div v-else-if="value.type == 'string' && !Array.isArray(value.enum) && (Object.keys(out).includes(key)||!readOnly)">
          <OpenApiEditString v-model:data="out[key]" :name="key" :required="Array.isArray(required) && required.includes(key)" :defaultdata="value.default" :description="value.description" :read-only="readOnly" />
        </div>
      </div>
    </div>
  </q-expansion-item>
