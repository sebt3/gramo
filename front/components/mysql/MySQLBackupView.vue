...
const { result, loading, onResult, onError } = useQuery(mysqlMySQLBackupQuery, {"namespace": navigation.currentNamespace, "name": navigation.currentItem }, { pollInterval: 500 });onError(onErrorHandler); onResult(res => {onNotMySQLBackupFound(res);viewerUpdate(res, res.data.mysqlMySQLBackup.metadata.obj)});
