...
export function useInnoDBCluster() {
  ...
  return {
    ...
    onNotInnoDBClusterFound: (res) => {
      if ( !res.loading && res.data.mysqlInnoDBCluster == null) {
    ...
    toView: (namespace, name) => router.push({ name: 'mysqlInnoDBClusterView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'mysqlInnoDBClusterView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'mysqlInnoDBClusterEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'mysqlInnoDBClusterCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> InnoDBCluster <strong>${row.metadata.name}</strong>`,
