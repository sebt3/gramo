...
export function useMySQLBackup() {
  ...
  return {
    ...
    onNotMySQLBackupFound: (res) => {
      if ( !res.loading && res.data.mysqlMySQLBackup == null) {
    ...
    toView: (namespace, name) => router.push({ name: 'mysqlMySQLBackupView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'mysqlMySQLBackupView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'mysqlMySQLBackupEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'mysqlMySQLBackupCreate', params: {namespace}}),
    actionDelete: (deletor, row) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> MySQLBackup <strong>${row.metadata.name}</strong>`,
