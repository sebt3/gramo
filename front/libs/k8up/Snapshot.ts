...
export { SnapshotListExcludes, SnapshotReadExcludes, SnapshotSimpleExcludes } from './custom.js'
import { extraSnapshotColumns } from './custom.js'
import { stringify, parse } from 'yaml'

export const SnapshotColumns:Array<QTableColumn> = ([
  {name: 'Name', label: 'Name', field: row => row.metadata.name, sortable: true, align: tableColumnAlign.left}
] as Array<QTableColumn>).concat(extraSnapshotColumns).concat([
...
export const SnapshotDefinition = {"description":"Snapshot is the Schema for the snapshots API","type":"object","properties":{"apiVersion":{"description":"APIVersion defines the versioned schema of this representation of an object. Servers should convert recognized schemas to the latest internal value, and may reject unrecognized values. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources","type":"string"},"kind":{"description":"Kind is a string value representing the REST resource this object represents. Servers may infer this from the endpoint the client submits requests to. Cannot be updated. In CamelCase. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds","type":"string"},"spec":{"description":"SnapshotSpec contains all information needed about a restic snapshot so it can be restored.","type":"object","properties":{"date":{"type":"string","format":"date-time"},"id":{"type":"string"},"paths":{"type":"array","items":{"type":"string"}},"repository":{"type":"string"}}},"status":{"description":"SnapshotStatus defines the observed state of Snapshot","type":"object"}},"x-kubernetes-group-version-kind":[{"group":"k8up.io","kind":"Snapshot","version":"v1"}]};
...
//      updateFromQuery: (res: object, obj: object) => {editor.value.ready=false;if(!res['loading']){editor.value.setSpec(obj);editor.value.ready=true;}},
  ...
  return {
    ...
    if ( !res.loading && res.data.k8sNamespace[0].k8upSnapshot == null) {
    ...
    actionDelete: (deletor, meta) => {
      $q.dialog({
        ...
        message: `<em>You are</em> about to <span class="text-red">delete</span> Snapshot <strong>${meta.name}</strong>`,
