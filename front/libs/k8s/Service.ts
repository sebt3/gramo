...
//      updateFromQuery: (res: object, obj: object) => {editor.value.ready=false;if(!res['loading']){editor.value.setSpec(obj);editor.value.ready=true;}},
  ...
  return {
    ...
    onNotServiceFound: (res) => {
    if ( !res.loading && res.data.k8sNamespace[0].k8sService == null) {
    ...
    toView: (namespace, name) => router.push({ name: 'k8sServiceView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8sServiceView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8sServiceEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8sServiceCreate', params: {namespace}}),
    actionDelete: (deletor, meta) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> Service <strong>${meta.name}</strong>`,
