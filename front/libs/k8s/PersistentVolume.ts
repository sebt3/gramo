...
//      updateFromQuery: (res: object, obj: object) => {editor.value.ready=false;if(!res['loading']){editor.value.setSpec(obj);editor.value.ready=true;}},
  ...
  return {
    ...
    onNotPersistentVolumeFound: (res) => {
    if ( !res.loading && res.data.k8sPersistentVolume == null) {
        const matched = router.currentRoute.value.matched
        notifyError('PersistentVolume not found');
        router.push({
    ...
    toView: (name) => router.push({ name: 'k8sPersistentVolumeView', params: {name}}),
    toViewReloaded: async (name) => {await router.push({ name: 'k8sPersistentVolumeView', params: {name}});router.go(0);},
    toEdit: (name) => router.push({ name: 'k8sPersistentVolumeEdit', params: {name}}),
    ...
    actionNew: ()    => router.push({ name: 'k8sPersistentVolumeCreate'}),
    actionDelete: (deletor, meta) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> PersistentVolume <strong>${meta.name}</strong>`,
