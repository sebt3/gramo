...
export function usePodTemplate() {
  ...
  const onlyReadProperties = (obj) => {
      ...
      Object.keys(obj).forEach(k=>{
        if (["template"].includes(k)||k=='metadata')
          res[k] = obj[k];
      })
      return res
    };
  ...
  return {
    onlyReadProperties, onlyWriteProperties, editor, viewer, viewerUpdate: (obj) => {
      ...
      viewer.value.props["template"]=gqlDataToYaml({"template": obj["template"]})
    },
    navigation: useNavigationStoreRef(),
    isNamespaced, setNamespaceFromRoute, setNamespacedItemFromRoute,
    router, pagination, setItemFromRoute, notify, notifySuccess, notifyError, notifyWorking, onErrorHandler,
    onNotPodTemplateFound: (res) => {
    if ( !res.loading && res.data.k8sNamespace[0].k8sPodTemplate == null) {
        const matched = router.currentRoute.value.matched
        notifyError('PodTemplate not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'k8sPodTemplateView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8sPodTemplateView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8sPodTemplateEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8sPodTemplateCreate', params: {namespace}}),
    actionDelete: (deletor, meta) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> PodTemplate <strong>${meta.name}</strong>`,
