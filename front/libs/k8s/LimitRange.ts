...
//      updateFromQuery: (res: object, obj: object) => {editor.value.ready=false;if(!res['loading']){editor.value.setSpec(obj);editor.value.ready=true;}},
  ...
  return {
    ...
    onNotLimitRangeFound: (res) => {
    if ( !res.loading && res.data.k8sNamespace[0].k8sLimitRange == null) {
        const matched = router.currentRoute.value.matched
        notifyError('LimitRange not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'k8sLimitRangeView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8sLimitRangeView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8sLimitRangeEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8sLimitRangeCreate', params: {namespace}}),
    actionDelete: (deletor, meta) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> LimitRange <strong>${meta.name}</strong>`,
