...
//      updateFromQuery: (res: object, obj: object) => {editor.value.ready=false;if(!res['loading']){editor.value.setSpec(obj);editor.value.ready=true;}},
  ...
  return {
    ...
    onNotReplicationControllerFound: (res) => {
    if ( !res.loading && res.data.k8sNamespace[0].k8sReplicationController == null) {
        const matched = router.currentRoute.value.matched
        notifyError('ReplicationController not found');
        router.push({
    ...
    toView: (namespace, name) => router.push({ name: 'k8sReplicationControllerView', params: {name, namespace}}),
    toViewReloaded: async (namespace, name) => {await router.push({ name: 'k8sReplicationControllerView', params: {name, namespace}});router.go(0);},
    toEdit: (namespace, name) => router.push({ name: 'k8sReplicationControllerEdit', params: {name, namespace}}),
    ...
    actionNew: (namespace)    => router.push({ name: 'k8sReplicationControllerCreate', params: {namespace}}),
    actionDelete: (deletor, meta) => {
      $q.dialog({
        title: 'Confirm <span class="text-red">delete</span>',
        message: `<em>You are</em> about to <span class="text-red">delete</span> ReplicationController <strong>${meta.name}</strong>`,
