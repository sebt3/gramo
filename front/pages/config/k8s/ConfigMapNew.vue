...
import ConfigMapNew from '@/queries/k8s/ConfigMap.create.graphql'
import OpenApiEdit from '@/components/core/OpenApiEdit.vue';
import MetadataNew from '@/components/core/MetadataNew.vue';
import { ref, useQuery, useMutation, useConfigMap, ConfigMapDefinition, sanitizeData, getProperties } from '@/libs/k8s/ConfigMap.js'
const name = ref('');
const binaryData  = ref({});
const data  = ref({});
const immutable  = ref({});
const { router, navigation, setNamespacedItemFromRoute, notifySuccess, notifyError, notifyWorking } = useConfigMap();setNamespacedItemFromRoute();
const { mutate, onDone, onError } = useMutation(ConfigMapNew);
onDone(() => {
...
function onSubmit() {
  ...
  mutate({
    ...
    "name": name.value,
    "binaryData": sanitizeData(binaryData.value),
    ...
    <q-card bordered class="q-ma-sm">
      ...
      <q-card-section>
        <OpenApiEdit
          ...
          :properties="getProperties(ConfigMapDefinition.properties.immutable)"
        />
      </q-card-section>
      <q-card-actions>
