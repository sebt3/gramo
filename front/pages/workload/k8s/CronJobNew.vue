...
import CronJobNew from '@/queries/k8s/CronJob.create.graphql'
import OpenApiEdit from '@/components/core/OpenApiEdit.vue';
import MetadataNew from '@/components/core/MetadataNew.vue';
import { ref, useQuery, useMutation, useCronJob, CronJobDefinition, sanitizeData, getProperties } from '@/libs/k8s/CronJob.js'
const name = ref('');
const spec  = ref({});
const { router, navigation, setNamespacedItemFromRoute, notifySuccess, notifyError, notifyWorking } = useCronJob();setNamespacedItemFromRoute();
const { mutate, onDone, onError } = useMutation(CronJobNew);
onDone(() => {
