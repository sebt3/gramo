...
import StatefulSetNew from '@/queries/k8s/StatefulSet.create.graphql'
import OpenApiEdit from '@/components/core/OpenApiEdit.vue';
import MetadataNew from '@/components/core/MetadataNew.vue';
import { ref, useQuery, useMutation, useStatefulSet, StatefulSetDefinition, sanitizeData, getProperties } from '@/libs/k8s/StatefulSet.js'
const name = ref('');
const spec  = ref({});
const { router, navigation, setNamespacedItemFromRoute, notifySuccess, notifyError, notifyWorking } = useStatefulSet();setNamespacedItemFromRoute();
const { mutate, onDone, onError } = useMutation(StatefulSetNew);
onDone(() => {
