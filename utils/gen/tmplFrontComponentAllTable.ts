...
let filter = ref('');
const { onErrorHandler, toView, toEdit, actionDelete, navigation, pagination } = use{{ name }}();
const { result, refetch, onError } = useQuery({{ mini }}All{{ name }}sQuery, {}, { pollInterval: 500 });onError(onErrorHandler);
</script>
<template>
  <q-card class="q-ma-sm" bordered>
    <TableHeader title="{{ short }} {{ name }}s" v-model:model-filter="filter" itemtype='{{ mini }} {{ name }}' @refresh="refetch()" :usecreate="false" />
    <q-card-section class="q-pa-none">
      <q-table v-if="result !== undefined" :rows="result.namespaces.map(x=>x.{{ mini }}{{ name }}s).flat()" :columns="{{ miniName }}AllColumns" class="no-shadow" v-model:pagination="pagination" :filter="filter" hide-bottom>
